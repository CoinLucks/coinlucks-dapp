...
import useBetStakingClaim from "@/hooks/game/useBetStakingClaim";
...
const StakeBtnClaim = () => {
  ...
  const { sendTransaction, hash, error, isError, isConfirming, isConfirmed } =
    useBetStakingClaim(chainId, gameName!);
  ...
  return (
    <Button
      ...
      radius="md"
      variant="flat"
      className="px-4 py-3 w-full bg-gradient-green text-white font-bold"
      isLoading={btnLoading}
      onClick={handleSubmit}
    >
      {btnLoading ? t("wallet_confirm") : t("btn_claim")}
    </Button>
  );
};

export default StakeBtnClaim;
