...
import useBetStakingUnstake from "@/hooks/game/useBetStakingUnstake";
...
const StakeBtnStake = () => {
  ...
  const { sendTransaction, hash, error, isError, isConfirming, isConfirmed } =
    useBetStakingUnstake(chainId, gameName!);
  ...
  return (
    <Button
      ...
      radius="md"
      variant="flat"
      className="px-4 py-3 w-full bg-gradient-yellow text-white font-bold"
      isLoading={btnLoading}
      onClick={handleSubmit}
    >
      {btnLoading ? t("wallet_confirm") : t("btn_unstake")}
    </Button>
  );
};
